...
#define SRAM_LENGTH ((20 * 1024)) /*Ram Length = 20KBs*/
...
u32 Vectors[] __attribute__((section(".isr_section"))) =
    {
        STACK_START,
        (u32)Reset_Handler,
        (u32)NMI_Handler,
...
void Reset_Handler(void)
{
    // reallocate .data from flash to ram
    // Calculate size of .data section
    u32 data_size = (u32)&e_data - (u32)&s_data;
    u32 *DataSrcPtr = (&(e_text) + 1); // sram
    u32 *DataDestAddress = &s_data;    // flash
    for (u32 i = 0; i < data_size; i++)
    {
        *DataDestAddress = *DataSrcPtr; /* copy from flash to ram*/
        DataSrcPtr++;
        DataDestAddress++;
    }
    // initialize .bss with zeroes
    u32 bss_size = (u32)&e_bss - (u32)&s_bss; /* how many bytes?*/
    u32 *BssDestAddress = &s_bss;
